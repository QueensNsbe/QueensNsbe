...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('a[href*="#"]').forEach(a => {
    ...
    if (!hash) return;
    const samePage = pagePart === '' || pagePart === window.location.pathname.split('/').pop();
    if (!samePage) return;
  ...
  if (calGrid) {
    const calEvents = [
      { date: '2025-10-25', title: 'Movie Night — Get Out' },
      { date: '2025-11-08', title: 'The "HUNT" Conference' },
      { date: '2025-11-12', title: 'NSBE Internship Panel' }
    ...
    function renderEventList() {
      ...
      calEvents.forEach(e => {
        ...
        item.innerHTML = `<div class="cal-event-date">${label}</div><div class="cal-event-title">${e.title}</div>`;
